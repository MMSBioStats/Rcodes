...
rm(list=ls())
# COMPARING QUADRATIC POLYNOMIAL TO QUADRATIC SPLINE
##### Suppose we have a predictor that takes the values 1:24
#
x <- c(1:24)
#
...
# Create quadratic term and fit polynomial
x.squared <- x^2

fit <- lm( y ~ x + x.squared )
#
summary( fit )
#
pred1 <- predict( fit )
...
# Fit b-spline quadratic
fit2 <- lm( y ~ bs( x ,degree =2) )
#
summary( fit2 )
pred2 <- predict( fit2 )
#
...
plot( x, y )
lines( x, mu, col="red" )
lines( x, pred2, col="blue", lwd=2 )

lines( x, pred1, col="green", lwd=2 )
