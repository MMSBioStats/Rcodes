...
set.seed(508329)
## specify frequencies of 6 (mRS=0,1,2,..,5+6) ordered categories
p1<-c(10,	3,	5,	6,	5,	16)
p2<-c(4,	7,	3,	21,	17,	25)
p3<-c(7,	14,	12,	8,	10,	12)
...
pocl_simulate <- function(n, a, b, levs=0:length(a)) {
  dat <- data.frame(Treatment=block_rand(n,3,LETTERS[1:3])) 
...
pocl_Bayes_test <- function(dat,priormu,priorvar) {
  ...
  probs<-matrix(NA,nrow=2,ncol=length(priormu))
  
  for(i in 1:length(priormu)){
  Dinv<-diag(c(1/priorvar[i],1/priorvar[i],rep(1/10,nlev-1)))
  # CHANGE TO MATRIX MULTIPLICATION HERE 
  
  Binv<-Sig_inv+Dinv
  B<-solve(Binv)
  
  Bb<-B%*%(Sig_inv%*%beta+Dinv%*%c(priormu[i],priormu[i],rep(0,nlev-1)))
...
resp <- factor(replicate(100000, sample(1:4, 1, prob=prbs)),
               ordered=TRUE, levels=1:4)
## fit POCL model; extract intercepts (zeta here)
alph <- polr(resp~1)$zeta
...
#Effect size
logOR<-log(1.5)  #log(1.78) for 0.7, log(1.56) for 0.75, log(1.5) for 0.80
...
#Effect size
logOR<-log(1.72) #log(1.78) for 0.7, log(1.56) for 0.75, log(1.5) for 0.80
